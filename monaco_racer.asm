...
; --- carburante (valori modificabili) ---
CH_FUEL     = 30                ; char della tanica (custom 30)
FUELMAX     = 240               ; capienza del serbatoio
FUELLOW     = FUELMAX/4         ; soglia di riserva (25%): sotto
                                ; questo valore compaiono le taniche
FUELRATE1   = 10                ; frame per unita' in prima marcia
FUELRATE2   = 6                 ; frame per unita' in seconda

; --- difficolta' progressiva ---
DIFFMAX     = 8                 ; livello massimo (resta giocabile:
                                ; al massimo 1 scorrimento a frame)
...
in_cc   lda $d000,x
        ...
        ; tile personalizzate: 27=mezzeria, 28=ciuffi, 29=ghiaia, 30=tanica
...
ng_t    sta halftab,x           ; strada larga ovunque
        lda #T_NORM
        sta typetab,x
        sta fueltab,x           ; nessuna tanica in pista
        lda #7
        dex
        bpl ng_t
...
mainloop
        ...
        ; scorrimento con difficolta' progressiva: passo base
        ; marcia*8 (8=ogni 2 frame, 16=ogni frame) + livello,
        ; con tetto a 16 (mai piu' di uno scorrimento a frame)
        lda speed
        asl
        asl
        asl
...
ml_p
        jsr printscore
        jsr checkcol
        jsr chkfuel             ; raccolta delle taniche
        jsr fueltick            ; consumo e barra del carburante
        jsr difftick            ; difficolta' progressiva
...
;--------------------------------------------------------------
scrollroad
        lda fueltab+24          ; tanica mai raccolta che esce
        beq sr_t0               ; dallo schermo in basso
        lda #0
        sta fuelonscr
sr_t0   ldx #23
sr_tab  lda halftab,x
        ...
        lda curtype
        sta typetab+1
        lda #0                  ; la riga nuova nasce senza tanica
        sta fueltab+1           ; (eventualmente la mette drawrow1)
...
dr_rd   sta SCREEN+40,x
        ...
        sta SCREEN+40,x
dr_x
        ; --- tanica di carburante: compare solo in riserva, una
        ;     alla volta, mai in galleria, sempre in carreggiata ---
        lda fuelcool            ; distanza minima dall'ultima
        beq fs_ck
        dec fuelcool
        jmp fs_x
fs_ck   lda fuelonscr           ; gia' una in pista?
        bne fs_x
        lda fuel
        cmp #FUELLOW            ; serbatoio sopra la riserva?
        bcs fs_x
        lda curtype
        cmp #T_TUNNEL           ; mai al buio
        beq fs_x
        jsr getrnd              ; scarto casuale dal centro
        and #7
        sec
        sbc #3                  ; -3..+4
        bne fs_o
        lda #2                  ; mai sulla mezzeria
fs_o    clc
        adc #CENTER
        sta tmpc
        lda curhalf             ; dentro la carreggiata corrente
        sec
        sbc #2
        sta tmpw
        lda #CENTER
...
fs_c1   lda #CENTER
        ...
        sta tmpc
fs_c2   lda tmpc
        sta fueltab+1           ; registra colonna e disegna
        tax
        lda #CH_FUEL
        sta SCREEN+40,x
        lda #1
        sta fuelonscr
        lda #100                ; righe prima della prossima
        sta fuelcool
fs_x    rts
...
updene
        ldx #2
ue_lp   lda difflevel           ; ai livelli alti il traffico
        lsr                     ; si avvicina piu' in fretta
        lsr
        lsr                     ; +0 o +1
        clc
        adc speed               ; e in base alla marcia
...
dc_lv   jsr printlives
        lda #FUELMAX            ; auto nuova, serbatoio pieno
        sta fuel
        jsr drawfuel
...
;--------------------------------------------------------------
; colonna video del giocatore: A = (X-12)/8
;--------------------------------------------------------------
getpcol
        lda pxl
        sec
...
;--------------------------------------------------------------
; RACCOLTA TANICA: se nelle righe sotto l'auto (21-23) c'e' una
; tanica entro una colonna di distanza, fa il pieno
;--------------------------------------------------------------
chkfuel
        lda fuelonscr
        beq cf_x
...
cf_r    lda fueltab,x
        ...
        clc
        adc #1
cf_a    cmp #2                  ; entro +/-1 colonna
        bcs cf_n
        ; --- presa! ---
        lda fueltab,x
        tay                     ; colonna della tanica
        lda #0
        sta fueltab,x
        sta fuelonscr
        lda rowlo-21,x          ; cancella la tanica dallo schermo
        sta DSTL
        lda rowhi-21,x
        sta DSTH
        lda typetab,x           ; rimette il fondo giusto
        cmp #T_ICE
        bne cf_t1
        lda #CH_BLUE
        bne cf_w
...
; indirizzi video delle righe 21-23 (per cancellare la tanica)
rowlo   !byte <(SCREEN+21*40),<(SCREEN+22*40),<(SCREEN+23*40)
rowhi   !byte >(SCREEN+21*40),>(SCREEN+22*40),>(SCREEN+23*40)

;--------------------------------------------------------------
; CONSUMO: il serbatoio cala (il doppio in seconda marcia);
; a secco vale come un incidente (e l'auto nuova ha il pieno)
;--------------------------------------------------------------
fueltick
        dec fuelcnt
        bne ft_d
...
ft_s    sta fuelcnt
        dec fuel
        bne ft_d
        jsr docrash             ; rimasto a secco!
ft_d    jmp drawfuel

;--------------------------------------------------------------
; barra del carburante (7 tacche, in alto a destra);
; in riserva lampeggia
;--------------------------------------------------------------
drawfuel
        lda fuel
        cmp #FUELLOW
        bcs df_ok
        lda frame               ; riserva: lampeggio
        and #8
        beq df_ok
        lda #0
        beq df_set
...
df_l    cpx tmpc
        bcs df_e
        lda #CH_RAIL            ; tacca piena
        bne df_w
df_e    lda #CH_ROAD            ; tacca vuota
df_w    sta SCREEN+33,x
        dex
        bpl df_l
        rts

; blip di rifornimento (voce 2)
fuelsnd
        lda #80
        sta $d408
        lda #$08
        ...
        lda #$10
        sta $d40b
        lda #$11                ; triangolo, gate on
        sta $d40b
        rts

;--------------------------------------------------------------
; DIFFICOLTA' PROGRESSIVA: +1 livello ogni ~10 secondi fino a
; DIFFMAX; accelera scorrimento e traffico restando giocabile
;--------------------------------------------------------------
difftick
        inc diffcnt
        bne df_x2
        inc diffcnt2
        lda diffcnt2
        cmp #2                  ; 2 x 256 frame = ~10 secondi
        bcc df_x2
        lda #0
        sta diffcnt2
        lda difflevel
...
ff_t3   lda txtvite,x
        ...
        dex
        bpl ff_t3
        lda #6                  ; "F" della barra carburante
        sta SCREEN+32
        rts
...
customchr
        ...
        !byte %10001000,%00000000,%00100010,%00000000
        ; char 30: tanica di carburante (con la "F")
        !byte %00111000,%01111100,%11111110,%10011110
        !byte %10111110,%10011110,%10111110,%11111110
...
halftab     !fill 25,7
typetab     !fill 25,0
; carburante e difficolta'
fuel        !byte 0             ; livello del serbatoio (0..FUELMAX)
fuelcnt     !byte 0             ; contatore del consumo
fuelonscr   !byte 0             ; 1 = tanica in pista
fuelcool    !byte 0             ; righe prima della prossima tanica
pcol        !byte 0             ; colonna del giocatore
difflevel   !byte 0             ; livello di difficolta' (0..DIFFMAX)
diffcnt     !byte 0
diffcnt2    !byte 0
fueltab     !fill 25,0          ; colonna della tanica per riga (0=no)
...
!byte $00,$01,$ff,$80,$07,$ff,$e0,$07
!byte $ff,$e0,$07,$ff,$e0,$07,$81,$e0
!byte $01,$81,$80,$01,$c3,$80,$01,$bd
!byte $80,$01,$bd,$80,$01,$ff,$80,$07
!byte $ff,$e0,$07,$bd,$e0,$07,$c3,$e0
!byte $07,$ff,$e0,$03,$ff,$c0,$00,$ff
!byte $00,$00,$42,$00,$00,$00,$00,$01
